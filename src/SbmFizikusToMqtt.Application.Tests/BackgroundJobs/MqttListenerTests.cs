...
public sealed class MqttListenerTests
{
    ...
    public async Task ExecuteAsync_SubscribesToCorrectTopicPattern()
    {
        ...
        var topicFilter = capturedOptions.TopicFilters.FirstOrDefault();
        Assert.NotNull(topicFilter);
        Assert.Equal("sbm/devices/+/set", topicFilter.Topic);
        Assert.Equal(MqttQualityOfServiceLevel.AtMostOnce, topicFilter.QualityOfServiceLevel);
    }
    ...
    public async Task HandleMessageReceived_NullDeserializedPayload_LogsWarningAndReturns()
    {
        ...
        // It only logs Warning when deserialization returns null (which doesn't happen with invalid JSON)
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Failed to parse JSON payload")),
                It.IsAny<Exception>(),
    ...
    public async Task ExecuteAsync_CustomSbmTopic_UsesCorrectTopicPattern()
    {
        ...
        var topicFilter = capturedOptions.TopicFilters.FirstOrDefault();
        Assert.NotNull(topicFilter);
        Assert.Equal("custom/topic/devices/+/set", topicFilter.Topic);
    }
